            ...
            <div className="flex gap-3">
              ...
              <Button type="default" loading={!!taskQueueStatus?.submitting} onClick={async () => {
                try {
                  ...
                  // 立即显示"正在提交"状态，在队列中显示一个临时任务
                  const tempTaskId = `submitting_${Date.now()}`;
                  const submittingConfig = {
                    encoder: matrixEncoder,
                    nvencCodec,
                  ...
                  setTaskQueueStatus(prev => ({
                    ...
                    submitting: { id: tempTaskId, config: submittingConfig }
                  }));
                  const FEISHU_WEBHOOK = 'https://open.feishu.cn/open-apis/bot/v2/hook/a2714380-7dcf-403e-924b-8af1aa146267';
                ...
                <div className="mb-2 p-2 bg-yellow-50 rounded border border-yellow-300">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <Text className="text-yellow-600 font-medium">提交中</Text>
                    </div>
                    <Text type="secondary" className="text-xs">正在上传视频文件...</Text>
                  </div>
                  <div className="flex gap-3 mt-1 flex-wrap text-xs text-gray-500">
                    <span>编码器: {taskQueueStatus.submitting.config?.encoder}{taskQueueStatus.submitting.config?.nvencCodec ? `(${taskQueueStatus.submitting.config.nvencCodec})` : ''}</span>
                    {taskQueueStatus.submitting.config?.presets && <span>presets: {taskQueueStatus.submitting.config.presets}</span>}
                    {taskQueueStatus.submitting.config?.rcMode && <span>rc: {taskQueueStatus.submitting.config.rcMode}</span>}
                    {taskQueueStatus.submitting.config?.cqValues && <span>cq: {taskQueueStatus.submitting.config.cqValues}</span>}
                    {taskQueueStatus.submitting.config?.qpValues && <span>qp: {taskQueueStatus.submitting.config.qpValues}</span>}
                    {taskQueueStatus.submitting.config?.bitrates && taskQueueStatus.submitting.config.bitrates !== '0' && <span>bitrate: {taskQueueStatus.submitting.config.bitrates}</span>}
                    {taskQueueStatus.submitting.config?.skipVmaf && <span className="text-orange-500">跳过VMAF</span>}
                  </div>
                </div>
              )}
